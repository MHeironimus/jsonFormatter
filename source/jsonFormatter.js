// Converts a JSON string into an HTML formatted string with collapsible
// members.
//
...
//------------------------------------------------------------------------------
$.jsonFormatter = function (options) {
    ...
    var _getRow = function (indent, data, isPropertyContent) {
        var tabs = "";
        for (var i = 0; i < indent && !isPropertyContent; i++) tabs += _options.tab;
    ...
    var _formatFunction = function (indent, obj) {
        var tabs = "";
        for (var i = 0; i < indent; i++) tabs += _options.tab;
    ...
    var _processObject = function (obj, indent, addComma, isArray, isPropertyContent) {
        ...
        if ($.isArray(obj)) {
            if (obj.length == 0) {
                ...
                clpsHtml = _options.collapsible ? "<span class='jsonFormatter-expander jsonFormatter-expanded'></span><span class='jsonFormatter-collapsible'>" : "";
                ...
                clpsHtml = _options.collapsible ? "</span>" : "";
    ...
    $(_options.selector).each(function(index, element) {
        _formatElement(element);
    });

    // Add Expander Click Event Handler
    $(_options.selector).on('click', '.jsonFormatter-expander', _expandImageClicked);
